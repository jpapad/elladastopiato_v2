...
export default function RecipeCard({ recipe }: { recipe: Recipe }) {
  ...
  return (
    ...
    >
      <Link href={`/recipes/${(recipe as any).slug || recipe.id}`} className="block">
        {/* Image Container */}
        <div className="relative h-72 w-full overflow-hidden">
          ...
          <div className="absolute top-6 left-6">
            <span className="px-4 py-2 bg-black/50 backdrop-blur-md border border-white/10 rounded-full text-[10px] font-black uppercase tracking-widest text-orange-500">
              {getCategoryLabel((recipe as any).recipeCategory) || 'Συνταγή'}
            </span>
          </div>
        </div>
        ...
        {/* Content */}
        <div className="p-8">
          <div className="flex items-center gap-3 text-black/30 dark:text-white/30 text-[10px] font-bold uppercase tracking-[0.2em] mb-4">
          ...
          <div className="flex items-center justify-between pt-6 border-t border-black/5 dark:border-white/5">
            <div className="flex items-center gap-4 text-black/40 dark:text-white/40 text-[10px] font-bold uppercase tracking-widest">
            ...
            <div className="w-10 h-10 rounded-full bg-black/5 dark:bg-white/5 flex items-center justify-center group-hover:bg-orange-500 group-hover:text-black transition-all duration-500">
              <ArrowRight size={18} />
            </div>
          </div>
        </div>
